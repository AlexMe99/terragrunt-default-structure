...
Templates enable reusable unit configurations, minimizing duplication while keeping setups standardized. A template can inherit variables or settings from parent stacks using Terragrunt functions such as:

```hcl
read_terragrunt_config(find_in_parent_folders("stack_level_a.hcl"))
````
...
Directory naming conventions help here: using Terragrunt/Terraform functions, you can dynamically derive stack names or regions from paths, e.g.:
...
* **Faster plan/apply times** (only relevant units are processed)
...
* **Caching efficiency:** Shared versions reuse cached binaries.
* **Simplified dependencies:** No need to manage multiple versions manually.
...
* Speed up module initialization
* Allow different provider versions per module
* Offer finer-grained control (e.g., credentials, endpoints)
...
* Keep higher-level configurations reusable and DRY
* Allow per-stack customization
* Scale organically and transparently

Avoid copy-pasting stacks — instead:

* Use **unit templates** as defaults
* Keep shared variables at higher stack levels for inheritance
* Continuously review and adapt variables and templates
...
Terraform modules are the building blocks of your infrastructure. How you organize and source them greatly affects maintainability and scalability.
...
* Early-stage projects or smaller teams

However, they lack versioning and cross-repo sharing capabilities, making them less suited for mature environments.

### Remote Modules

For mature projects or shared infrastructure, prefer **remote modules**, hosted in:

* Git repositories
* Terraform Cloud
* Private registries

Remote modules provide:

* Version control and stability
* Reuse across multiple repositories
* Safer collaboration through controlled updates
...
By keeping your setup **DRY**, organizing stacks and variables thoughtfully, leveraging **remote state** and **provider caching**, and combining **templating** with **modular Terraform design**, you enable teams to manage complex infrastructures confidently and efficiently.
